<x-app-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="p-6 text-gray-900">
            <form action="{{ route('sub-contractor-bills.update', $subContractorBill) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm form-control @error('bill_no') is-invalid @enderror" id="bill_no" name="bill_no"
                            value="{{ old('bill_no', $subContractorBill->bill_no) }}" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm form-control @error('amount') is-invalid @enderror" id="amount" name="amount"
                            value="{{ old('amount', $subContractorBill->amount) }}" step="0.01" min="0" required>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm form-control @error('date') is-invalid @enderror" id="date" name="date"
                            value="{{ old('date', $subContractorBill->date->format('Y-m-d')) }}" required>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-12">
                        ...
                        <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm form-control" id="remark" name="remark" rows="3"
                            placeholder="Additional remarks...">{{ old('remark', $subContractorBill->remark) }}</textarea>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-warning">
                        <i class="fas fa-save"></i> Update Bill & Transaction
                    </button>
                </div>
            </form>
        </div>
