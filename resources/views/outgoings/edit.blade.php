<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <!-- Back Button -->
            <a href="{{ route('outgoings.index') }}" class="btn btn-outline-secondary">
                <i class="fas fa-arrow-left"></i> Back
            </a>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Outgoing Entry') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <form method="POST" action="{{ route('outgoings.update', $outgoing) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="outgoing-btn">
                    ...
                    <button type="submit" class="btn btn-primary btn-sm">Update Outgoing Entry</button>
                </div>
            </form>
        </div>
    </div>
