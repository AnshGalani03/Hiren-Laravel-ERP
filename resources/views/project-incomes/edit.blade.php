<x-app-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <form method="POST" action="{{ route('project-incomes.update', $projectIncome) }}">
                @csrf
                @method('PATCH')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control @error('incoming_id') is-invalid @enderror" id="incoming_id" name="incoming_id" required>
                            ...
                            <option value="{{ $incoming->id }}"
                                {{ old('incoming_id', $projectIncome->incoming_id) == $incoming->id ? 'selected' : '' }}>
                                {{ $incoming->name }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('projects.show', $projectIncome->project) }}" class="btn btn-secondary">Cancel</a>
