<x-app-layout>
    <x-slot name="header">
        <div class="module-edit-page d-flex justify-content-between align-items-center">
            <a href="{{ route('employees.show', $upad->employee) }}" class="btn btn-outline-secondary">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <form method="POST" action="{{ route('upads.update', $upad) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="employee_id" value="{{ $upad->employee_id }}">
                ...
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('employees.show', $upad->employee) }}" class="btn btn-secondary btn-sm">Cancel</a>
                    <button type="submit" class="btn btn-primary btn-sm">Update Upad</button>
                </div>
            </form>
        </div>
    </div>
